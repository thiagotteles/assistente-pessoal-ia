...
- [ ] Criar arquivo de configuração `.assistant-core/agents/arquiteto.md` (agente conversacional)
- [ ] Definir personalidade confrontadora construtiva e técnica
- [ ] Configurar YAML embarcado com comandos e personalidade
- [ ] Implementar sistema de memória contextual em `.assistant-core/memory/arquiteto-memory.yaml`
- [ ] Testar carregamento de configuração básica via `/arquiteto`

### Tarefa 2: Sistema de Consultoria Técnica com Memória (AC: 1, 3)
- [ ] Implementar task `consultoria-tecnica.md` para discussões técnicas
- [ ] Criar lógica de recuperação de decisões anteriores via [[referências]]
- [ ] Desenvolver algoritmo de confronto construtivo baseado em histórico
- [ ] Implementar carregamento automático de contexto técnico
- [ ] Testar confronto baseado em decisões reais vs genérico

### Tarefa 3: Sistema de Análise de Código e Projetos (AC: 2, 5)
- [ ] Implementar task `analise-projeto.md` para review de código/arquitetura
- [ ] Criar workflow de questionamento ativo ("Por que não X?", "E se Y?")
- [ ] Desenvolver identificação de padrões arquiteturais
- [ ] Implementar detecção de desvios de padrões
- [ ] Testar análise com URLs/repositórios reais

### Tarefa 4: Integração com Knowledge-Base Técnica (AC: 4, 6)
- [ ] Criar template `.assistant-core/templates/decisao-tecnica.yaml`
- [ ] Implementar task `registrar-decisao.md` para persistência de decisões
- [ ] Desenvolver sistema de cross-referencing automático [[projetos]] e [[pessoas]]
- [ ] Implementar consolidação em arquivos únicos `decisao-*.md`
- [ ] Integrar com Knowledge-Base Manager para persistência

### Tarefa 5: Tracking e Evolução de Decisões (AC: 6)
- [ ] Implementar task `historico-decisoes.md` para visualização de evolução
- [ ] Criar algoritmo de detecção de inconsistências
- [ ] Desenvolver sistema de aprendizado de preferências arquiteturais
- [ ] Implementar alertas proativos sobre conflitos com decisões anteriores
- [ ] Testar evolução de padrões ao longo do tempo

### Tarefa 6: Cross-Agent Intelligence e Colaboração (AC: 4)
- [ ] Implementar colaboração com Mentor para alinhamento técnico/carreira
- [ ] Criar integração com Secretária para contexto de projetos
- [ ] Desenvolver compartilhamento de insights técnicos entre agentes
- [ ] Implementar sincronização com arquivos únicos do sistema
- [ ] Testar colaboração entre agentes

### Tarefa 7: Testes e Validação do Sistema (Todos ACs)
- [ ] Criar script de teste `tests/test-story-2.3.sh` (seguindo guideline)
- [ ] Testar todos os fluxos principais do arquiteto
- [ ] Validar confronto construtivo vs confronto genérico
- [ ] Testar integração com sistema existente (Stories 2.1 e 2.2)
- [ ] Executar testes de memória e recuperação de contexto histórico

### Tarefa 8: Deploy e Instalação no Claude Code (Todos ACs)
- [ ] **CRÍTICO**: Atualizar `scripts/deploy-assistant.sh` com novo agente arquiteto
- [ ] **CRÍTICO**: Documentar instalação no Claude Code (slash command `/arquiteto`)
- [ ] Criar documentação de uso do agente arquiteto
- [ ] Testar deploy limpo em pasta temporária
- [ ] Validar que agente carrega corretamente após instalação
